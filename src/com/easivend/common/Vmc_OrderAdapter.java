...
public class Vmc_OrderAdapter
{
    ...
    int[] RealStatusvalue;// 退款状态，0不显示未发生退款动作，1退款完成，2部分退款，3退款失败
  	...
  	{  		
  	    ...
  	    //数字类型订单信息
  	    payTypevalue = new int[listinfos.size()];
		payStatusvalue = new int[listinfos.size()];
		RealStatusvalue = new int[listinfos.size()];
  	    ...
  	    for (Tb_vmc_order_pay tb_inaccount : listinfos) 
  	    {
  			...
  			payStatusvalue[m] = tb_inaccount.getPayStatus();
  			RealStatusvalue[m] = tb_inaccount.getRealStatus();
	...
	public void setPayStatusvalue(int[] payStatusvalue) {
		this.payStatusvalue = payStatusvalue;
	}

	public int[] getRealStatusvalue() {
		return RealStatusvalue;
	}

	public void setRealStatusvalue(int[] realStatusvalue) {
		RealStatusvalue = realStatusvalue;
	}
